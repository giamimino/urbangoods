fetch('../../globalPHP/databaseGAD.php')
    .then(response => {
        if (!response.ok) {
    ...
    .then(jsons => {
        ...
        jsons.forEach(json => {
            if (json.category === "Tech Gadgets") {
                const productWrapper = document.createElement("a");
                productWrapper.href = `../../product/php/product.php?refresh=true&id=${json.id}`;
                productWrapper.classList.add("product-wrapper");
                containerProducts.appendChild(productWrapper);
                productWrapper.addEventListener("click", ()=> {
                    window.location.href = `../../product/php/product.php?refresh=true&id=${json.id}`; // Loads the new page and refreshes everything
                });
